...
Midnight currently does not support contract-to-contract calls, meaning the vault cannot call into a separately deployed contract to verify or execute token transfers within the Midnight ecosystem. The deeper constraint, however, is the cross-chain gap between Midnight and Polygon: the underlying asset is USDC on Polygon, and Midnight has no bridge to Polygon and no mechanism for a Compact circuit to observe or verify external chain state. Both constraints are addressed in Section 10, which explains the two-step deposit design they necessitate and how the architecture evolves as Midnight's cross-chain capabilities mature.
...
| Individual deposit amount | Private | Auth key is a one-way hash of amount, user key, and nonce |
| Deposit authorization nonce | Private | Auth key is a one-way hash of amount, user key, and nonce |
...
| Manager-gated capital return | manager_deposit restricted to manager |
